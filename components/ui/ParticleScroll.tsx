// implementacion con background transparente
"use client"
import {
  useEffect,
  useRef,
...
void main () {
  ...
  // Solo pintamos donde el contenido realmente está formado (vis) y tiene
  // su propio alpha (tex.a). En cualquier otro caso queda transparente
  // de verdad, dejando ver lo que haya detrás del canvas (p.ej. un fondo
  // animado propio) en vez de un color de fondo estimado y opaco.
  outColor = vec4(tex.rgb, vis * tex.a * uCover);
}`;
...
void main () {
  ...
  float vis = (1.0 - step(0.9995, t)) * step(home.x, uMaxX * uRes.x) * step(home.y, uRes.y) * step(-uDensity, home.y);
  if (vis < 0.5) {
  ...
  pos += vec2(h4 - 0.5, h1 - 0.5) * uDensity * 3.0 * (1.0 - smoothstep(0.5, 0.85, t));
